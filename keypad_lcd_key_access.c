...
bool ProcessKeypadInput(void) {
	...
	if(key && !keypadState.isComplete && keypadState.position < ACCESS_KEY_SIZE) {
		...
		// For debugging: Print received key
...
int main(void) {
	...
	while(1) {
		...
		// Receive access key
		if(HAL_UART_Receive(&huart1, receivedAccessKey, ACCESS_KEY_SIZE, 1000) == HAL_OK) {
			printf("Access key received: ");
			for(int i = 0; i < ACCESS_KEY_SIZE; i++) {
				printf("%c", receivedAccessKey[i]);
			}
			printf("\r\n");
			...
			// Reset keypad state
			memset(&keypadState, 0, sizeof(KeypadInput));
			// Wait for correct access key
			while(!keypadState.isVerified) {
				if(ProcessKeypadInput()) {
					// Receive timestamp
					if(HAL_UART_Receive(&huart1, (uint8_t*)&timestamp, sizeof(timestamp), 1000) != HAL_OK) {
						printf("Failed to receive timestamp\r\n");
						continue;
					}

					// Receive data size
					if(HAL_UART_Receive(&huart1, (uint8_t*)&data_size, sizeof(data_size), 1000) != HAL_OK ||
					...
					while(received < data_size) {
						...
						if(HAL_UART_Receive(&huart1, &encrypted_buffer[received], chunk_size, 1000) != HAL_OK) {
					...
					if(received == data_size) {
						// Receive end marker (0x55)
						uint8_t endMarker;
						if(HAL_UART_Receive(&huart1, &endMarker, 1, 1000) == HAL_OK && endMarker == 0x55) {
							printf("Data reception complete\r\n");

							// Derive decryption key and decrypt data
							deriveKeyFromAccessKey(receivedAccessKey, timestamp, decryption_key);
							decryptData(encrypted_buffer, data_size, decryption_key);

							// Display decrypted text
							displayTextOnLCD((char*)encrypted_buffer, data_size);
						} else {
